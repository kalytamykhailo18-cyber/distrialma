...
const ICON_MAP: Record<string, IconType> = {
  ...
  picking: HiOutlineClipboardList,
...
export default function Navbar() {
  ...
  const isVendedorOnly = isStaff && permissions?.length === 1 && permissions[0] === "vendedor";
  const NAV_TABS = isStaff
    ? isVendedorOnly
      ? [{ href: "/admin/vendedor", label: "Tomar Pedido", match: "/admin/vendedor" }]
  ...
  return (
    <nav className="bg-white shadow-md border-b border-gray-200 sticky top-0 z-40">
      <div className="max-w-7xl mx-auto px-4 py-2 flex items-center justify-between gap-4">
        {/* Logo */}
        <Link href={isVendedorOnly ? "/admin/vendedor" : isStaff ? "/admin" : "/"} className="shrink-0">
