...
import { PageTransition, Stagger, LoadingCenter, springBtn, hoverRow, useDataReady, staggerStyle } from "@/components/AnimateIn";
...
export default function DiasEntregaPage() {
  ...
  const dataReady = useDataReady(clients.length > 0 ? clients : null);
      ...
      <Stagger delay={100}>
        <div className="flex gap-2 mb-4">
          <input
            ...
            className="flex-1 px-4 py-2 border border-brand-400 rounded-xl text-sm focus:outline-none focus:border-brand-600 focus:ring-1 focus:ring-brand-600"
          />
          <button
              ...
              <tbody className="divide-y">
                {clients.map((client, idx) => (
                  <tr
                    key={client.cod}
                    className={hoverRow}
                    style={staggerStyle(dataReady, idx, 200, 25)}
                  >
                    ...
                    {DAYS.map((day) => (
                      <td key={day} className="text-center px-1 py-2">
                        <button
                          ...
                          className={`w-7 h-7 rounded-md text-xs font-bold ${springBtn} ${
