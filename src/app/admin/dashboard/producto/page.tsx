...
export default function ProductoDetailPage() {
  ...
  return (
    <PageTransition className="max-w-4xl mx-auto px-4 py-6">
      <Stagger delay={0} y={-8}>
        <Link href="/admin/resumen-productos" className={`inline-flex items-center gap-1 text-sm text-brand-600 hover:text-brand-700 mb-4 ${springBtn}`}>
          ...
          <div className="divide-y">
            {data.topClientes.map((c, i) => (
              <Link key={c.clienteCod} href={`/admin/dashboard/cliente?cod=${c.clienteCod}`} target="_blank"
                className={`px-4 py-2.5 flex items-center justify-between block ${hoverRow}`} style={staggerStyle(true, i)}>
