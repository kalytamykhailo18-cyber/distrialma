...
export default function PreciosMasivosPage() {
  ...
  return (
    <PageTransition className="max-w-4xl mx-auto px-4 py-6">
        ...
        <div className="bg-white border rounded-xl shadow-sm p-4 mb-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                <input type="number" value={porcentaje} onChange={(e) => { setPorcentaje(e.target.value); setPreview([]); setResult(""); }}
                  ...
                  className="flex-1 px-3 py-2 border border-brand-400 rounded-xl text-sm text-right font-bold" />
          ...
          <div className="flex gap-2">
            <button onClick={handlePreview} disabled={loading || !porcentaje || (!marcaCod && !proveedorCod)}
              className={`px-4 py-2 bg-blue-500 text-white rounded-xl text-sm font-medium disabled:opacity-50 ${springBtn}`}>
              {loading ? "Buscando..." : "Vista previa"}
            </button>
            {preview.length > 0 && !confirmStep && (
              <button onClick={() => setConfirmStep(true)}
                className={`px-4 py-2 bg-brand-500 text-white rounded-xl text-sm font-medium ${springBtn}`}>
            ...
            {confirmStep && (
              <div className="flex items-center gap-2">
                ...
                <button onClick={handleApply} disabled={applying}
                  className={`px-4 py-2 bg-red-600 text-white rounded-xl text-sm font-medium disabled:opacity-50 ${springBtn}`}>
                  {applying ? "Aplicando..." : "Si, aplicar"}
                </button>
                <button onClick={() => setConfirmStep(false)}
                  className={`px-4 py-2 bg-gray-200 text-gray-700 rounded-xl text-sm font-medium ${springBtn}`}>
                  Cancelar
                </button>
              </div>
            )}
      ...
      {loading && <LoadingCenter text="Buscando productos..." />}
        ...
        <Stagger delay={150}>
          <div className="bg-white border rounded-xl shadow-sm overflow-hidden">
            ...
            <div className="divide-y max-h-[400px] overflow-y-auto">
              {preview.map((p, i) => (
                <div key={p.sku} className={`px-4 py-2 flex items-center justify-between ${hoverRow}`} style={staggerStyle(true, i)}>
